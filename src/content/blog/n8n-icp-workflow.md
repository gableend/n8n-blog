...
This is the story of how, in just a few hours, I used n8n to automate ICP creation, combining TheirStack, OpenAI, and trusty Google tools.
...
- **n8n**: An intuitive, low-code workflow automation platform, allowing me to visually orchestrate tasks between APIs and tools.
...
<p style="text-align: center;"><b>n8n Workflow Canvas</b></p>
...
Of course, things are rarely straightforward out of the gate. There was some initial messing about figuring out how to obtain my API Key and correctly add it to the node configuration. After a bit of trial and error, I got the connection working, only to realize I needed a paid TheirStack subscription to fully enable the API. Subscription sorted, I successfully pulled the data using the handy "Test Workflow" feature built into n8n.
...
At this stage, things got a little tricky. I wanted to populate 500 records, the maximum my TheirStack account would allow via the API (you have to be a superuser to do more), so I would have a representative set of n8n customers from major economies. I had some difficulties because I couldn't get my workflow to loop through each record in turn. I initially tried a 'Loop Over Items' node to solve this, but made a breakthrough when I realized I could share the input and output data from a node with ChatGPT and provide some context. ChatGPT understood my issue and suggested using a Code node to break down my large dataset into individual records.
...
Soon I hit another roadbump in that OpenAI was struggling to make sense of multiple datasets and would appear to hang while thinking. With some help from n8n's AI assistant (which accesses documentation and community content) I was able to create a simple input for the LLM with an "Edit Fields" node and another Code node.
...
I did the summarizing, ICP analysis, and Google Doc steps in a separate workflow.. I'm sure I could have done it in one, but breaking it up kept the overall layout manageable.
...
My ICP analysis was simple, focusing only on firmographic data, as a proof of concept. In reality, to build out the full ICP, I would be bringing in multiple 1st party data sources, to augment 3rd party, and analyzing technographics, psychographics, business situation and operating model too. 
